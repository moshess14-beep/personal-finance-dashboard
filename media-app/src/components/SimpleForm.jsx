...
import { TYPE_LABEL } from '../data/constants'
...
// טופס הוספה לבילוי / מתכון / מוצר — עם התמונה שהועלתה והצעות מהזיהוי
export default function SimpleForm({ type, init, imageUrl, titleSuggestions, onSave, busy }) {
  ...
  function save() {
    ...
    onSave({
      ...
      mapsUrl: type === 'place' && f.title ? mapsSearchUrl(`${f.title} ${f.address}`.trim()) : '',
      region: f.region,
  ...
  return (
    <div className="space-y-3">
      ...
      <Field
        label={`שם ה${TYPE_LABEL[type]} *`}
        value={f.title}
        onChange={set('title')}
        placeholder={
          type === 'place' ? 'למשל: חוף הבונים' : type === 'recipe' ? 'למשל: פסטה ברוטב עגבניות' : 'למשל: מחבת גריל'
        }
      />
      ...
      {type === 'place' && (
        <>
          <div>
            <label className="text-[11px] font-bold text-slate-400">כתובת / אזור</label>
      ...
      {type === 'product' && (
        <>
          ...
          <Field
            ...
            dir="ltr"
          />
          <ToggleChips label="קטגוריה" options={PRODUCT_CATEGORIES} selected={f.productCategory} onToggle={(opt) => setF((s) => ({ ...s, productCategory: s.productCategory === opt ? null : opt }))} single />
        </>
      )}
      ...
      <button
        onClick={save}
        disabled={busy}
        className="w-full bg-indigo-600 disabled:bg-slate-300 text-white font-bold rounded-2xl py-3 active:scale-[0.99] transition flex items-center justify-center gap-2"
      >
