...
// חיפוש הצעות מחיר למוצר (עם עיגון בחיפוש גוגל): עד 5 הצעות מחנויות ישראליות,
// מהזולה ליקרה, עם ציון אמינות. המחירים הם מה שנמצא ברשת כרגע — הצעה לבדיקה, לא התחייבות.
export async function aiFindOffers(product, apiKey) {
  if (DEMO) return []
  if (!apiKey && !aiProxy)
    throw new AiError('הזיהוי החכם לא זמין — התחברו לחשבון או הזינו מפתח', 'no key & no proxy')

  const prompt = `חפש בגוגל היכן קונים בישראל את המוצר: "${product.title}"${product.kind ? ` (${product.kind})` : ''}.
מצא עד 5 הצעות מחיר עדכניות מחנויות אונליין ישראליות, כולל עלות משלוח אם היא מופיעה.
החזר JSON בלבד:
{"offers":[{"store":"שם החנות","price":המחיר בש"ח כמספר,"shippingNote":"עלות/תנאי המשלוח אם ידועים (למשל: משלוח חינם, ‎+30 ₪), אחרת ריק","link":"קישור לעמוד המוצר בחנות","trust":"high|medium|low"}]}
כללים: סדר מהזולה ליקרה. trust=high רק לחנויות מוכרות ואמינות (רשתות גדולות, אתר היצרן, חנויות ותיקות). אל תמציא מחיר או קישור — אם אינך בטוח שההצעה אמיתית ועדכנית, השמט אותה. אם לא נמצא כלום: {"offers":[]}.`

  const data = await geminiJson(apiKey, [{ text: prompt }], 4096, true)
  return (Array.isArray(data?.offers) ? data.offers : [])
    .filter((o) => o && o.store && o.price != null)
    .slice(0, 5)
}
